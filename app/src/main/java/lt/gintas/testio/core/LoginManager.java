...
public class LoginManager {
    ...
    @Inject
    public LoginManager(Application application, AppPreferences preferences, AppService appService, NetworkStateManager networkStateManager, Gson gson) {
    ...
    public Flowable<Boolean> login(final String phone, final String pass, final boolean rememberMe) {
        return Flowable.just(mNetworkStateManager.isNetworkOnline()).flatMap(new Function<Boolean, Flowable<Boolean>>() {
            @Override
            ...
            public Flowable<?> apply(Flowable<? extends Throwable> errorNotification) {
                return errorNotification
                        .zipWith(Flowable.range(1, 5), new BiFunction<Throwable, Integer, Pair<Throwable, Integer>>() {
                            @Override
                            public Pair<Throwable, Integer> apply(Throwable throwable, Integer attempts) {
            ...
            public Flowable<Boolean> apply(Boolean aBoolean) {
                ...
                return mAppService.login(new LoginRequest(phone, pass)).flatMap(new Function<LoginResponse, Flowable<Boolean>>() {
                    @Override
